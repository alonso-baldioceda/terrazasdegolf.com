...
AWS_BUCKET_NAME=$(grep AWS_BUCKET_NAME .env.production | cut -d '=' -f2);
...
aws s3 rm s3://$AWS_BUCKET_NAME/ --recursive && npm run clean && npm run build && aws s3 sync public s3://$AWS_BUCKET_NAME/ --acl public-read;